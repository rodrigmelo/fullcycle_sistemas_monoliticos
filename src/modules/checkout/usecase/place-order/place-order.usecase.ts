...
export default class PlaceOrderUseCase implements UseCaseInterface {
  ...
  async execute(input: PlaceOrderInputDto): Promise<PlaceOrderOutputDto> {
    ...
    const payment = await this._paymentFacade.process({
      orderId: customOrder.id.id,
      amount: customOrder.total,
    });
    ...
    payment.status === "approved" && customOrder.approved();
    this._repository.addOrder(customOrder);

    return {
      id: customOrder.id.id,
      invoiceId: payment.status === "approved" ? invoice.id : null,
      status: customOrder.status,
      total: customOrder.total,
      products: customOrder.products.map((product) => {
        return {
